...
def run_experiment(run_name, out_dir='./results', seed=None,
                   ...
                   hidden_dims=[1024], ycn=False, custom_model=None,
                   **kw):
    """
    ...
    # Select model class (experiment 1 or 2 or custom_model)
    if ycn and custom_model:
        raise Exception("No support for running several models")
    ...
    if not custom_model:
      model = model_cls(in_size, num_classes, filters=filters,
   ...
   # optimizer = torch.optim.SGD(model.parameters(), lr=lr,momentum=0.9, weight_decay=reg)
    optimizer = torch.optim.Adam(model.parameters(), lr=lr, betas=(0.9, 0.999), weight_decay=reg)
    trainer = training.TorchTrainer(model, loss_fn, optimizer, device)
    fit_res = trainer.fit(dl_train, dl_val,
                          num_epochs=epochs,
