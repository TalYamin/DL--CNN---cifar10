...
class TorchTrainer():
    ...
    def fit(self, dl_train: DataLoader, dl_val: DataLoader,
        ...
        if early_stopping is not None:
          test_loss_improvement = EarlyStopping(early_stopping, 0.01)
        ...
        for epoch in range(num_epochs):
            ...
            validation_result = self.test_epoch(dl_val)
            val_loss += validation_result.losses
            val_acc.append(validation_result.accuracy)
            actual_num_epochs += 1
            epochs_without_improvement = test_loss_improvement(sum(train_result.losses) / len(train_result.losses))
            if test_loss_improvement.early_stop:
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        loss_tensor = self.loss_fn(outputs,y)
        loss = loss_tensor.item()
        loss_tensor.backward()
        self.optimizer.step()
        num_correct = self.get_correct_predictions(outputs, y)
        # ========================

        return BatchResult(loss, num_correct)
    
    @staticmethod
    def get_correct_predictions(y,t):
      return (torch.max(y, dim=1).indices == t).sum().item()
